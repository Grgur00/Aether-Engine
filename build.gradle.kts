...
val schemaInit =
    tasks.register("aetherSchemaInit") {
        group = "aether schema"
        description =
            "Proposes initial committed schema locks under each project's build directory."
    }

val schemaUpdate =
    tasks.register("aetherSchemaUpdate") {
        group = "aether schema"
        description = "Proposes schema-lock updates without modifying committed locks."
    }

val schemaAccept =
    tasks.register("aetherSchemaAccept") {
        group = "aether schema"
        description =
            "Explicitly accepts generated schema proposals into source-controlled directories."
    }

val schemaCheck =
    tasks.register("aetherSchemaCheck") {
        group = "verification"
...
subprojects {
    plugins.withId("java") {
        ...
        fun proposalTask(name: String, descriptionText: String) =
            tasks.register<JavaCompile>(name) {
                ...
                options.generatedSourceOutputDirectory =
                    layout.buildDirectory.dir("aether-schema/generated")
                options.compilerArgs.addAll(
                    listOf(
                        ...
                        "-Aaether.schemaProposalDirectory=" +
                            proposalDirectory.get().asFile.absolutePath,
                    )
                )
