...
public final class SocialNetworkApplication {
    ...
    public static void main(String[] arguments) {
        try (TypedAetherDatabase database =
                arguments.length == 0
            ...
            social.feedFor("usr-1002")
                    .forEach(
                            post -> {
                                UserProfile author =
                                        social.findProfile(post.authorId()).orElseThrow();
                                System.out.println(
                                        "  @" + author.username() + ": " + post.content());
                            });

            System.out.println(
                    "\nDeleted draft exists: " + social.findPost("post-draft").isPresent());
        }
    }
