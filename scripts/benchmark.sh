...
database_directory="${AETHER_BENCHMARK_DIR:-$(
    mktemp -d "${TMPDIR:-/tmp}/aether-cv-benchmark.XXXXXX"
)}"
...
echo "Use a release JDK, close heavy applications, and avoid quoting results from a" \
    "thermally throttled run."

if [[ "${cache_mode}" == "cold-open" ]]; then
    echo "Cold-open mode purges the macOS filesystem cache system-wide and may temporarily" \
    ...
    if [[ "${AETHER_ALLOW_BATTERY:-0}" != "1" ]] && pmset -g batt | grep -q "Battery Power"; then
        echo "Refusing cold-open benchmark on battery power. Connect the charger or set" \
            "AETHER_ALLOW_BATTERY=1." >&2
        exit 2
    fi
fi

cd "${repository_root}"
exec ./gradlew :modules:aether-benchmarks:run \
    --args="--directory ${database_directory} --output ${result_file} \
--records ${record_count} --reads ${read_count} --crash-points ${crash_points} \
--batch-size ${batch_size} --value-bytes ${value_bytes} --durability ${durability} \
