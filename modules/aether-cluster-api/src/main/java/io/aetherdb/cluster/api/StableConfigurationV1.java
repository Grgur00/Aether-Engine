...
/** First stable cluster-configuration representation. */
public final class StableConfigurationV1 implements StableConfiguration {
    private final UUID clusterId, configurationId;
    private final long version, created;
    private final byte[] previousHash, hash;
    private final List<ClusterMember> members;
    private final Set<UUID> voters;

    /**
     * Creates and canonicalizes a stable configuration.
     *
    ...
    public StableConfigurationV1(
        ...
        if (clusterId == null
                ...
                || configurationId.equals(new UUID(0, 0))
                || version < 1
                || created < 0)
            throw new IllegalArgumentException("invalid configuration identity");
        if (previousHash == null || previousHash.length != 32 || hash == null || hash.length != 32)
            throw new IllegalArgumentException("hashes must be 32 bytes");
        this.members =
                members.stream().sorted((a, b) -> compareUuid(a.nodeId(), b.nodeId())).toList();
        Set<UUID> ids = new HashSet<>();
        Set<ClusterEndpoint> eps = new HashSet<>();
        int vc = 0, sc = 0;
        for (ClusterMember m : this.members) {
            if (!ids.add(m.nodeId())) throw new IllegalArgumentException("duplicate node");
            for (var e : m.endpoints())
                if (!eps.add(e))
                    throw new IllegalArgumentException("duplicate endpoint across nodes");
            if (m.role() == MemberRole.VOTER) vc++;
            else sc++;
        }
        if (vc < 1 || vc > 31 || sc > 64)
    ...
    /**
     * Compares UUIDs by their unsigned 128-bit representation.
     *
     * @param a first UUID
     * @param b second UUID
     * @return negative, zero, or positive according to unsigned ordering
     */
    ...
    public long creationEpochMillis() {
        return created;
    }
