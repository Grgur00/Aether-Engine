...
public final class ClusterEndpoint implements Comparable<ClusterEndpoint> {
    ...
    public enum Scheme {
        ...
        AETHER_TLS(1),
        /** Unencrypted transport intended only for development. */
        AETHER_TCP_DEVELOPMENT(2);
     ...
     */
    public static ClusterEndpoint of(
            Scheme scheme, String host, int port, int priority, int flags) {
        if (host == null || host.isBlank()) throw new IllegalArgumentException("host is blank");
        if (host.equals("0.0.0.0") || host.equals("::") || host.equals("*"))
            throw new IllegalArgumentException("wildcard host is not advertisable");
        ...
        try {
            if (candidate.indexOf(':') >= 0)
                return new ClusterEndpoint(
                        scheme,
                        AddressType.IPV6,
                        InetAddress.getByName(candidate).getAddress(),
                        port,
                        priority,
                        flags);
            if (candidate.matches("[0-9]+(?:\\.[0-9]+){3}")) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    if (v > 255) throw new IllegalArgumentException("invalid IPv4 host");
                    bytes[i] = (byte) v;
                }
        ...
        return new ClusterEndpoint(
                scheme,
                AddressType.DNS,
                dns.getBytes(StandardCharsets.US_ASCII),
                port,
                priority,
                flags);
    }
    ...
    public ClusterEndpoint(
            Scheme scheme, AddressType type, byte[] address, int port, int priority, int flags) {
        if (scheme == null || type == null || address == null)
            throw new IllegalArgumentException("endpoint fields are required");
        if (port < 1 || port > 65535 || priority < 0 || priority > 65535)
            throw new IllegalArgumentException("invalid port or priority");
        if ((flags & ~(INTERNAL_RPC | CLIENT_RPC)) != 0 || flags == 0)
            throw new IllegalArgumentException("invalid endpoint flags");
        int required = type == AddressType.IPV4 ? 4 : type == AddressType.IPV6 ? 16 : -1;
        if ((required > 0 && address.length != required)
                || (type == AddressType.DNS && (address.length < 1 || address.length > 255)))
     ...
     *
     * @return defensive copy of canonical address bytes
     */
    public byte[] address() {
     ...
     *
     * @return endpoint ordering priority
     */
    public int priority() {
        return priority;
    }
     ...
     *
     * @return supported RPC endpoint flags
     */
    public int flags() {
        return flags;
    }
    ...
    @Override
    public int compareTo(ClusterEndpoint other) {
        return COMPARATOR.compare(this, other);
    }

    /** Canonical comparator used by durable configuration encodings. */
    public static final Comparator<ClusterEndpoint> COMPARATOR =
            Comparator.comparingInt(ClusterEndpoint::priority)
                    .thenComparingInt(e -> e.scheme.code())
                    .thenComparingInt(e -> e.addressType.code())
                    .thenComparing(ClusterEndpoint::address, Arrays::compareUnsigned)
                    .thenComparingInt(ClusterEndpoint::port)
                    .thenComparingInt(ClusterEndpoint::flags);

    @Override
    public boolean equals(Object o) {
        return o instanceof ClusterEndpoint e && compareTo(e) == 0;
    }

    @Override
    public int hashCode() {
        return (((((scheme.hashCode() * 31 + addressType.hashCode()) * 31
