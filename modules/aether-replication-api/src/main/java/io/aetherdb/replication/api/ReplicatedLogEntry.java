...
public record ReplicatedLogEntry(
    ...
    public ReplicatedLogEntry {
        ...
        if (type == null
                ...
                || commandId == null
                || sequenceStart < 0
                || stateSequenceAfter < 0
                || previousEntryHash == null
                || previousEntryHash.length != 32
                || entryHash == null
                || entryHash.length != 32
                || payload == null
                || payload.length > 67_108_664
                || type == ReplicatedEntryType.COMMAND
                        && (commandId.equals(zero)
                                || payloadFormatVersion != 1
                                || sequenceStart < 1
                                || stateSequenceAfter < sequenceStart
                                || payload.length == 0)
        ...
        entryHash = Arrays.copyOf(entryHash, 32);
        payload = Arrays.copyOf(payload, payload.length);
    }

    /** Returns the previous hash defensively. */
    @Override
    public byte[] previousEntryHash() {
        return previousEntryHash.clone();
    }

    /** Returns this entry hash defensively. */
    @Override
    public byte[] entryHash() {
        return entryHash.clone();
    }

    /** Returns exact payload bytes defensively. */
    @Override
    public byte[] payload() {
    ...
    public boolean equals(Object other) {
        return this == other
                || other instanceof ReplicatedLogEntry entry
                        && type == entry.type
                        && payloadFormatVersion == entry.payloadFormatVersion
                        && index == entry.index
                        && term == entry.term
                        && commandId.equals(entry.commandId)
                        ...
                        && Arrays.equals(entryHash, entry.entryHash)
    ...
    public int hashCode() {
        ...
        result = 31 * result + Arrays.hashCode(entryHash);
