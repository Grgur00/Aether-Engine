...
public final class WalFragmentCodec {
     ...
     * assembled record are never returned.</p>
     *
     * @param physical physical bytes after the segment header
     * @param startOffset file offset corresponding to {@code physical[0]}
     * @return immutable recovered records, the exclusive verified file offset, and an optional issue
     */
        ...
        try {
            while (cursor < physical.length) {
                ...
                if (blockRemaining <= WalFormatV1.FRAGMENT_HEADER_BYTES) {
                    ...
                    cursor += available; fileOffset += available;
                    if (assembling == null) lastCompleteOffset = fileOffset;
                    continue;
                }
                if (physical.length - cursor < WalFormatV1.FRAGMENT_HEADER_BYTES) {
                    return recovery(records, lastCompleteOffset, "incomplete fragment header at offset " + fileOffset);
                }
                byte[] headerBytes = Arrays.copyOfRange(physical, cursor, cursor + WalFormatV1.FRAGMENT_HEADER_BYTES);
                boolean zero = true; for (byte value : headerBytes) zero &= value == 0;
                if (zero) throw corrupt("large zero fragment header");
                ...
                long record = Integer.toUnsignedLong(header.getInt());
                int fragment = Short.toUnsignedInt(header.getShort()), flags = Short.toUnsignedInt(header.getShort());
                if (length == 0 || length > blockRemaining - WalFormatV1.FRAGMENT_HEADER_BYTES
                        || cursor + (long) WalFormatV1.FRAGMENT_HEADER_BYTES + length > physical.length) {
                    return recovery(records, lastCompleteOffset, "incomplete fragment payload at offset " + fileOffset);
                }
                byte[] complete = Arrays.copyOfRange(physical, cursor,
                ...
                if (version != 1 || flags != 0 || record != expectedRecord || fragment != expectedFragment) {
                    throw corrupt("fragment metadata mismatch");
                }
                byte[] payload = Arrays.copyOfRange(complete, WalFormatV1.FRAGMENT_HEADER_BYTES, complete.length);
                if (type == WalFormatV1.FULL && assembling == null && fragment == 0) records.add(payload);
                else if (type == WalFormatV1.FIRST && assembling == null && fragment == 0) {
                    assembling = new ByteArrayOutputStream(); assembling.writeBytes(payload);
                } else if (type == WalFormatV1.MIDDLE && assembling != null) assembling.writeBytes(payload);
                else if (type == WalFormatV1.LAST && assembling != null) {
                    assembling.writeBytes(payload); records.add(assembling.toByteArray()); assembling = null;
                } else throw corrupt("invalid fragment sequence");
                expectedFragment++;
                cursor += complete.length; fileOffset += complete.length;
                if (type == WalFormatV1.FULL || type == WalFormatV1.LAST) {
                    expectedRecord++; expectedFragment = 0; lastCompleteOffset = fileOffset;
                }
            }
            String issue = assembling == null ? null : "incomplete logical record at offset " + lastCompleteOffset;
            return recovery(records, lastCompleteOffset, issue);
        } catch (WalCorruptionException failure) {
            return recovery(records, lastCompleteOffset, failure.getMessage() + " at offset " + fileOffset);
        }
    }

    private static PrefixRecovery recovery(List<byte[]> records, long validEndOffset, String issue) {
        return new PrefixRecovery(records.stream().map(byte[]::clone).toList(), validEndOffset, issue);
    }
    ...
    public record PrefixRecovery(List<byte[]> records, long validEndOffset, String issue) {
        ...
        @Override public List<byte[]> records() { return records.stream().map(byte[]::clone).toList(); }

        /** Reports whether bytes after the verified prefix were incomplete or corrupt. */
        public boolean hasIssue() { return issue != null; }
    }
