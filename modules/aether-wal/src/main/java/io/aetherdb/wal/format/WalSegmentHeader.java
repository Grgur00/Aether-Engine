...
public record WalSegmentHeader(UUID databaseId, long segmentNumber, long previousSegmentNumber,
    ...
    public byte[] encodeBlock() {
        ...
        bytes.put(MAGIC).putShort((short) 1).putShort((short) 96).putInt(WalFormatV1.BLOCK_BYTES)
                .putLong(WalFormatV1.SEGMENT_CAPACITY).putLong(databaseId.getMostSignificantBits())
                .putLong(databaseId.getLeastSignificantBits()).putLong(segmentNumber).putLong(previousSegmentNumber)
                .putLong(firstSequence).putLong(creationEpochMillis).putInt(0).put(new byte[16]);
    ...
    public static WalSegmentHeader decode(byte[] block, UUID expectedDatabase, long expectedSegment) {
        ...
        ByteBuffer bytes = ByteBuffer.wrap(block).order(ByteOrder.LITTLE_ENDIAN);
        bytes.position(8);
        if (bytes.getShort() != 1 || bytes.getShort() != 96 || bytes.getInt() != WalFormatV1.BLOCK_BYTES
                || bytes.getLong() != WalFormatV1.SEGMENT_CAPACITY) throw corrupt("unsupported WAL header");
        UUID id = new UUID(bytes.getLong(), bytes.getLong());
        long segment = bytes.getLong(), previous = bytes.getLong(), first = bytes.getLong(), created = bytes.getLong();
        if (bytes.getInt() != 0) throw corrupt("unknown WAL flags");
        for (int index = 76; index < 92; index++) if (block[index] != 0) throw corrupt("non-zero header reserved bytes");
        bytes.position(92);
        if (bytes.getInt() != MaskedCrc32c.masked(block, 0, 92)) throw corrupt("WAL header checksum mismatch");
        for (int index = 96; index < block.length; index++) if (block[index] != 0) throw corrupt("non-zero header block padding");
        if (!id.equals(expectedDatabase) || segment != expectedSegment || segment <= 0 || previous < 0 || first <= 0 || created < 0)
            throw corrupt("WAL header identity mismatch");
