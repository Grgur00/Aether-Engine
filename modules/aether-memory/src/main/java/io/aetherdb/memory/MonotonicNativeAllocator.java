...
/**
 * Aligned bump allocator over a fixed native region; individual allocations remain valid until the
 * allocator is closed.
 */
final class MonotonicNativeAllocator implements NativeAllocator {
    ...
    MonotonicNativeAllocator(FfmNativeRegion region) {
        this.region = region;
    }
    ...
    public Allocation tryAllocate(long sizeBytes, int alignmentBytes) {
        ...
        while (true) {
            ...
            long end;
            try {
                end = Math.addExact(aligned, sizeBytes);
            } catch (ArithmeticException overflow) {
                throw new IllegalArgumentException("allocation overflow", overflow);
            }
    ...
    public long usedBytes() {
        return cursor.get() - RegionConfig.FIRST_ALLOCATABLE_OFFSET;
    }

    @Override
    public long remainingBytes() {
        return region.capacityBytes() - cursor.get();
    }
    ...
    @Override
    public long alignmentPaddingBytes() {
        return padding.get();
    }
    ...
    @Override
    public long failedAllocationCount() {
        return failures.get();
    }
}
