...
public final class NativeRecordWriter {
    ...
    public NativeRecordWriter(NativeRegion region) {
        this.region = region;
    }
    ...
    private NativeAllocator.Allocation write(byte[] key, byte[] value, long sequence, byte type) {
        ...
        NativeAllocator.Allocation allocation =
                region.allocator().tryAllocate(total, NativeRecordFormatV1.ALIGNMENT);
    ...
    public void writeValueAt(int offset, byte[] key, byte[] value, long sequence) {
        ...
        writeAt(
                offset,
                NativeRecordFormatV1.totalLength(key.length, value.length),
                key,
                value,
                sequence,
                NativeRecordFormatV1.VALUE);
    }

    public void writeTombstoneAt(int offset, byte[] key, long sequence) {
        writeAt(
                offset,
                NativeRecordFormatV1.totalLength(key.length, 0),
                key,
                new byte[0],
                sequence,
                NativeRecordFormatV1.TOMBSTONE);
    }

    private void writeAt(
            int offset, int total, byte[] key, byte[] value, long sequence, byte type) {
        ...
        if (type == NativeRecordFormatV1.VALUE) {
            NativeAccess.copyFromArray(
                    value, record, NativeRecordFormatV1.HEADER_BYTES + key.length);
        }
        ...
        NativeAccess.setShort(
                record, NativeRecordFormatV1.FORMAT_VERSION_OFFSET, NativeRecordFormatV1.VERSION);
