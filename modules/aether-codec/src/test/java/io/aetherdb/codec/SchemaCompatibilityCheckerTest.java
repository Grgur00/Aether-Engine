...
class SchemaCompatibilityCheckerTest {
    ...
    @Test void acceptsStableRenameAndOptionalAddition() {
        byte[] versionOne = descriptor(1,
                "field=16|name|STRING|100|false\n");
        byte[] versionTwo = descriptor(2,
    ...
    @Test void rejectsRequiredAdditionRemovalTypeChangeAndTighterBound() {
        byte[] original = descriptor(1, "field=16|name|STRING|100|false\n");

        assertThatThrownBy(() -> SchemaCompatibilityChecker.requireCompatible(original,
                descriptor(2, "field=16|name|STRING|100|false\nfield=17|required|STRING|20|false\n")))
                .hasMessageContaining("new field 17 is required");
        assertThatThrownBy(() -> SchemaCompatibilityChecker.requireCompatible(original, descriptor(2, "")))
                .hasMessageContaining("field 16 was removed");
        assertThatThrownBy(() -> SchemaCompatibilityChecker.requireCompatible(original,
                descriptor(2, "field=16|name|UUID|16|false\n")))
                .hasMessageContaining("changed type");
        assertThatThrownBy(() -> SchemaCompatibilityChecker.requireCompatible(original,
                descriptor(2, "field=16|name|STRING|99|false\n")))
                .hasMessageContaining("tightened its bound");
    }

    @Test void rejectsChangedEnumOrNestedSchemaMeaningAtAStableFieldId() {
        byte[] original = descriptor(1,
                "field=16|status|ENUM|5|false\ndetail=16|enum|1:OPEN,2:CLOSED\n");
        byte[] changed = descriptor(2,
