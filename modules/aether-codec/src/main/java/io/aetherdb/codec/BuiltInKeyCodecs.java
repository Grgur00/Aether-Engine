...
public final class BuiltInKeyCodecs {
     ...
     */
    public static OrderedKeyCodec<String> utf8String() {
        return new Base<String>("aether:utf8", 65517, Comparator.naturalOrder()) {
            public byte[] encode(String v) {
                if (v == null) throw new IllegalArgumentException("key is null");
     ...
     */
    public static OrderedKeyCodec<Long> signedLong() {
        return new Base<Long>("aether:i64", 8, Comparator.naturalOrder()) {
            public byte[] encode(Long v) {
                return ByteBuffer.allocate(8).putLong(v ^ Long.MIN_VALUE).array();
            }

            public Long decode(byte[] b) {
                if (b.length != 8) throw new IllegalArgumentException("invalid long key");
                return ByteBuffer.wrap(b).getLong() ^ Long.MIN_VALUE;
            }
        };
    }
     ...
     */
    public static OrderedKeyCodec<UUID> uuid() {
        return new Base<UUID>(
            ...
            public UUID decode(byte[] b) {
                if (b.length != 16) throw new IllegalArgumentException("invalid UUID key");
     ...
     */
    public static <K> OrderedKeyCodec<K> forType(Class<K> type) {
        Objects.requireNonNull(type, "type");
        OrderedKeyCodec<?> codec;
        if (type == String.class) codec = utf8String();
        else if (type == Long.class || type == long.class) codec = signedLong();
        else if (type == UUID.class) codec = uuid();
        else
            throw new IllegalArgumentException(
                    "CODEC_NOT_AVAILABLE for key type " + type.getName());
        @SuppressWarnings("unchecked")
        OrderedKeyCodec<K> typed = (OrderedKeyCodec<K>) codec;
        return typed;
    }

    private abstract static class Base<T> implements OrderedKeyCodec<T> {
        private final String id;
        private final int max;
        private final Comparator<T> cmp;

        Base(String i, int m, Comparator<T> c) {
            id = i;
            max = m;
            cmp = c;
        }
        ...
        public byte[] fingerprint() {
            try {
                return MessageDigest.getInstance("SHA-256")
                        .digest((id + ":v1").getBytes(StandardCharsets.UTF_8));
