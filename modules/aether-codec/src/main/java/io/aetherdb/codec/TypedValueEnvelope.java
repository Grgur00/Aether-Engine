...
public final class TypedValueEnvelope {
    ...
    public static <V> byte[] encode(ValueCodec<V> c, V value) {
        ...
        return ByteBuffer.allocate(HEADER_BYTES + payload.length)
                ...
                .putLong(c.schemaId().getMostSignificantBits())
                .putLong(c.schemaId().getLeastSignificantBits())
                .putInt(c.currentSchemaVersion())
                .putInt(0)
    ...
    public static <V> V decode(ValueCodec<V> c, byte[] in) {
        ...
        b.get(magic);
        if (!Arrays.equals(magic, "AETV".getBytes(StandardCharsets.US_ASCII))
                || b.getShort() != 1
                || b.getShort() != HEADER_BYTES
                || b.getLong() != c.schemaId().getMostSignificantBits()
