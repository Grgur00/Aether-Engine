...
public final class SchemaCompatibilityChecker {
     ...
     * optional field required, or adding a required field requires an explicit migration.</p>
     *
    ...
    public static void requireCompatible(byte[] olderDescriptor, byte[] newerDescriptor) {
        ...
        for (Field previous : older.fields.values()) {
            ...
            if (!previous.type.equals(current.type)) throw incompatible("field " + previous.id + " changed type");
            if (!older.details.getOrDefault(previous.id, "").equals(
                    newer.details.getOrDefault(previous.id, ""))) {
                throw incompatible("field " + previous.id + " changed enum or nested-schema identity");
            }
            if (current.bound < previous.bound) throw incompatible("field " + previous.id + " tightened its bound");
            if (current.maximumEntries < previous.maximumEntries) {
                throw incompatible("field " + previous.id + " tightened its entry bound");
            }
            if (previous.optional && !current.optional) throw incompatible("field " + previous.id + " became required");
        }
        for (Field current : newer.fields.values()) {
            if (!older.fields.containsKey(current.id) && !current.optional) {
                throw incompatible("new field " + current.id + " is required and has no generated default");
            }
        }
    }
    ...
    private static Descriptor parse(byte[] encoded) {
        ...
        String schemaId = null; int version = 0; Map<Integer, Field> fields = new HashMap<>();
        ...
        try {
            for (String line : lines) {
                if (line.startsWith("schemaId=")) schemaId = line.substring("schemaId=".length());
                else if (line.startsWith("version=")) version = Integer.parseInt(line.substring("version=".length()));
                else if (line.startsWith("field=")) {
                    String[] values = line.substring("field=".length()).split("\\|", -1);
                    if (values.length < 4 || values.length > 6) throw incompatible("malformed field descriptor");
                    ...
                    int maximumEntries = values.length == 6 ? Integer.parseInt(values[5]) : Integer.MAX_VALUE;
                    if (id < 16 || bound < 0 || fields.putIfAbsent(id,
                            new Field(id, values[2], bound, optional, maximumEntries)) != null) {
                        throw incompatible("invalid or duplicate field identity");
                    }
                } else if (line.startsWith("detail=")) {
                    String[] values = line.substring("detail=".length()).split("\\|", 3);
                    if (values.length != 3 || details.putIfAbsent(Integer.parseInt(values[0]),
        ...
        if (schemaId == null || version < 1) throw incompatible("descriptor identity is incomplete");
        if (!fields.keySet().containsAll(details.keySet())) throw incompatible("detail references unknown field");
    ...
    private record Descriptor(String schemaId, int version, Map<Integer, Field> fields,
