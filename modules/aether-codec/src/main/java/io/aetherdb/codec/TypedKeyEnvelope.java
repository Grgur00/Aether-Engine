...
public final class TypedKeyEnvelope {
    ...
    public static <K> byte[] encode(CollectionDefinition<K, ?> d, K key) {
        ...
        return ByteBuffer.allocate(PREFIX_BYTES + user.length)
    ...
    public static byte[] prefix(CollectionDefinition<?, ?> d) {
        return ByteBuffer.allocate(PREFIX_BYTES)
                .order(ByteOrder.BIG_ENDIAN)
                .put((byte) 0x40)
                .putLong(d.id().value().getMostSignificantBits())
                .putLong(d.id().value().getLeastSignificantBits())
                .putShort((short) d.keyCodec().encodingVersion())
                .array();
    }
