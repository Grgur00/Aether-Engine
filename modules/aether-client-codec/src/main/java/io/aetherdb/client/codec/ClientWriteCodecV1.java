...
public final class ClientWriteCodecV1 {
    ...
    public static byte[] encode(ClientWriteRequest request) {
        int regionBytes = 0;
        long keys = 0, values = 0;
        for (var op : request.operations()) {
            regionBytes = Math.addExact(regionBytes, 16 + op.key().length + op.value().length);
            keys += op.key().length;
            values += op.value().length;
        }
        if (regionBytes > 32 * 1024 * 1024)
            throw new IllegalArgumentException("write body too large");
        byte[] out = new byte[128 + regionBytes];
        ByteBuffer b = ByteBuffer.wrap(out).order(ByteOrder.BIG_ENDIAN);
        b.putInt(0x41454357)
                .putShort((short) 1)
                .putShort((short) 128)
                .putInt(0)
                .putInt(request.operations().size())
                .putInt(regionBytes)
                .putInt(1)
        ...
        b.position(hashPosition + 32 + 20);
        b.putInt(0);
        b.position(128);
        int ordinal = 0;
        for (var op : request.operations()) {
            b.put((byte) (op.type() == ClientWriteOperation.Type.PUT ? 1 : 2))
                    .put((byte) 0)
                    .putShort((short) 0)
                    .putInt(op.key().length)
                    .putInt(op.value().length)
                    .putInt(ordinal++)
                    .put(op.key())
                    .put(op.value());
        }
        byte[] hash = sha(Arrays.copyOfRange(out, 128, out.length));
        System.arraycopy(hash, 0, out, 72, 32);
        ByteBuffer.wrap(out)
    ...
    public static ClientWriteRequest decode(byte[] in) {
        ...
        if (b.getInt() != 0x41454357 || b.getShort() != 1 || b.getShort() != 128 || b.getInt() != 0)
            throw invalid();
        ...
        b.get(reserved);
        if (!Arrays.equals(reserved, new byte[20])
                || !MessageDigest.isEqual(hash, sha(Arrays.copyOfRange(in, 128, in.length))))
            throw invalid();
        b.position(128);
        List<ClientWriteOperation> ops = new ArrayList<>();
        long actualKeys = 0, actualValues = 0;
        for (int i = 0; i < count; i++) {
            if (b.remaining() < 16) throw invalid();
            int type = b.get() & 255;
            if (b.get() != 0 || b.getShort() != 0) throw invalid();
            int kl = b.getInt(), vl = b.getInt(), ordinal = b.getInt();
            if (kl < 0 || vl < 0 || ordinal != i || b.remaining() < kl + vl) throw invalid();
            byte[] key = new byte[kl], value = new byte[vl];
            b.get(key).get(value);
            ops.add(
                    new ClientWriteOperation(
                            type == 1
                                    ? ClientWriteOperation.Type.PUT
                                    : type == 2 ? ClientWriteOperation.Type.DELETE : null,
                            key,
                            value));
            actualKeys += kl;
            actualValues += vl;
    ...
    private static IllegalArgumentException invalid() {
        return new IllegalArgumentException("invalid CLIENT_WRITE body");
    }
}
