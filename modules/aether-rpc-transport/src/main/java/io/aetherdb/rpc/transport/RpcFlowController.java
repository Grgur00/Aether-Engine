...
public final class RpcFlowController {
    ...
    public RpcFlowController(long initialCredit) {
        if (initialCredit < 1024 * 1024 || initialCredit > MAXIMUM_WINDOW)
            throw new IllegalArgumentException("invalid initial receive window");
    ...
    public synchronized void update(int delta) {
        if (delta <= 0 || credit > MAXIMUM_WINDOW - delta)
            throw new IllegalArgumentException("window update overflow");
        credit += delta;
    }
     ...
     */
    public synchronized long availableCredit() {
        return credit;
    }
}
