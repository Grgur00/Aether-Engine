...
public final class RpcMessageAssembler {
    ...
    public byte[] accept(RpcFrame frame) {
        ...
        if (header.beginsMessage()) {
            if (output != null) throw new RpcProtocolException("message BEGIN while assembly is active");
            if (header.messageLength() > maximumMessageBytes) throw new RpcProtocolException("message exceeds negotiated limit");
            type = header.type(); streamId = header.streamId(); code = header.code(); invocation = header.invocationId(); declaredLength = header.messageLength();
        ...
        if (header.type() != type || header.streamId() != streamId || header.code() != code || !header.invocationId().equals(invocation))
            throw new RpcProtocolException("fragment identity changed");
        if (header.fragmentOffset() != output.size() || (long) output.size() + header.payloadLength() > declaredLength)
