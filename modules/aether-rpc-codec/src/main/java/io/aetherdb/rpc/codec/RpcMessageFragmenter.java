...
public final class RpcMessageFragmenter {
     ...
     */
    public static List<RpcFrame> fragment(
            RpcFrameType type,
            long streamId,
            int code,
            UUID invocationId,
            ...
            byte[] message,
            int maximumPayload) {
        if (type != RpcFrameType.REQUEST && type != RpcFrameType.RESPONSE)
            throw new IllegalArgumentException("only request/response messages are fragmented");
        if (message == null
                || message.length > RpcFrameHeaderV1.MAX_MESSAGE_BYTES
                || maximumPayload < 1
                || maximumPayload > RpcFrameHeaderV1.MAX_FRAME_PAYLOAD)
            throw new IllegalArgumentException("invalid fragmentation limits");
        if (message.length == 0)
            return List.of(
                    frame(
                            type,
    ...
    private static RpcFrame frame(
        ...
        return new RpcFrame(
                new RpcFrameHeaderV1(
                        ...
                        begin ? total : 0,
                        offset,
                        type == RpcFrameType.REQUEST && begin ? timeout : 0,
                        0,
                        invocation),
                payload);
    }
