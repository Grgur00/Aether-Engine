...
public final class RpcFrameDecoder {
    ...
    public List<RpcFrame> feed(ByteBuffer input) {
        ...
        while (input.hasRemaining()) {
            if (headerBytes < header.length) {
                ...
                input.get(header, headerBytes, copied); headerBytes += copied;
                if (headerBytes < header.length) break;
                int length = validateHeaderAndPayloadLength(); payload = new byte[length]; payloadBytes = 0;
                if (length == 0) completed.add(complete());
            } else {
                int copied = Math.min(input.remaining(), payload.length - payloadBytes);
                input.get(payload, payloadBytes, copied); payloadBytes += copied;
    ...
    public boolean hasPartialFrame() { return headerBytes != 0; }

    /** Returns retained bytes for connection-level memory accounting. */
    public int retainedBytes() { return headerBytes + payloadBytes; }

    /** Drops any incomplete frame after terminal connection close. */
    public void reset() { headerBytes = 0; payload = null; payloadBytes = 0; }

    private int validateHeaderAndPayloadLength() {
        if (header[0] != 'A' || header[1] != 'E' || header[2] != 'R' || header[3] != 'P'
                || Byte.toUnsignedInt(header[4]) != 1 || header[5] != 0) {
    ...
    private RpcFrame complete() {
        ...
        try { return RpcFrameCodecV1.decode(encoded); }
        finally { headerBytes = 0; payload = null; payloadBytes = 0; }
    }
}
