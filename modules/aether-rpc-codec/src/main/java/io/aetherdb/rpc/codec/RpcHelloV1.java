 ...
 */
public record RpcHelloV1(
        Role role,
        UUID clusterId,
        UUID nodeId,
        UUID sessionId,
        long connectionNonce,
        int maximumFramePayload,
        int maximumMessageBytes,
        int maximumConcurrentStreams,
        int initialReceiveWindowBytes,
        int keepaliveIdleMillis,
    ...
    public enum Role {
        /** Peer that initiated the transport connection. */
        DIALER(1),
        /** Peer that accepted the transport connection. */
        ACCEPTOR(2);
    ...
    public RpcHelloV1 {
        if (role == null
                ...
                || isZero(sessionId)
                || connectionNonce == 0
                || maximumFramePayload < 1
                || maximumFramePayload > RpcFrameHeaderV1.MAX_FRAME_PAYLOAD
                || maximumMessageBytes < 0
                || maximumMessageBytes > RpcFrameHeaderV1.MAX_MESSAGE_BYTES
                || maximumConcurrentStreams < 1
                || maximumConcurrentStreams > 1024
                || initialReceiveWindowBytes < 1024 * 1024
                || initialReceiveWindowBytes > 256 * 1024 * 1024
                || keepaliveIdleMillis <= 0
                || keepaliveTimeoutMillis <= 0
                || engineMajor < 0
                || engineMinor < 0
                || enginePatch < 0
                || javaMajor < 21) throw new IllegalArgumentException("invalid RPC HELLO values");
    }
    ...
    public byte[] encode() {
        ...
        ByteBuffer bytes = ByteBuffer.wrap(result).order(ByteOrder.BIG_ENDIAN);
        bytes.put("AEHL".getBytes(StandardCharsets.US_ASCII))
                .putShort((short) 1)
                .putShort((short) ENCODED_LENGTH);
        bytes.put((byte) 1)
                .put((byte) role.code())
                .putShort((short) 0)
                .putShort((short) 0)
                .putShort((short) 0);
        bytes.putLong(0).putLong(0);
        putUuid(bytes, clusterId);
        putUuid(bytes, nodeId);
        putUuid(bytes, sessionId);
        bytes.putLong(connectionNonce)
                .putInt(maximumFramePayload)
                .putInt(maximumMessageBytes)
                .putInt(maximumConcurrentStreams);
        bytes.putInt(initialReceiveWindowBytes)
                .putInt(keepaliveIdleMillis)
                .putInt(keepaliveTimeoutMillis);
        bytes.putInt(engineMajor).putInt(engineMinor).putInt(enginePatch).putInt(javaMajor);
        bytes.put(compatibilityFingerprint());
        bytes.position(188).putInt(MaskedCrc32c.masked(result, 0, 188));
        return result;
    }
    ...
    public static RpcHelloV1 decode(byte[] encoded) {
        if (encoded == null || encoded.length != ENCODED_LENGTH)
            throw new RpcProtocolException("HELLO must be exactly 192 bytes");
        ByteBuffer bytes = ByteBuffer.wrap(encoded).order(ByteOrder.BIG_ENDIAN);
        byte[] magic = new byte[4];
        bytes.get(magic);
        if (!Arrays.equals(magic, "AEHL".getBytes(StandardCharsets.US_ASCII))
        ...
        int roleCode = Byte.toUnsignedInt(bytes.get());
        Role role = roleCode == 1 ? Role.DIALER : roleCode == 2 ? Role.ACCEPTOR : null;
        if (role == null
                || bytes.getShort() != 0
                || bytes.getShort() != 0
                || bytes.getShort() != 0
        ...
        UUID cluster = getUuid(bytes), node = getUuid(bytes), session = getUuid(bytes);
        long nonce = bytes.getLong();
        int frame = bytes.getInt(),
                message = bytes.getInt(),
                streams = bytes.getInt(),
                window = bytes.getInt();
        int idle = bytes.getInt(),
                timeout = bytes.getInt(),
                major = bytes.getInt(),
                minor = bytes.getInt(),
                patch = bytes.getInt(),
                java = bytes.getInt();
        ...
        for (int index = 160; index < 188; index++)
            if (encoded[index] != 0) throw new RpcProtocolException("nonzero HELLO reserved byte");
        int crc = ByteBuffer.wrap(encoded, 188, 4).order(ByteOrder.BIG_ENDIAN).getInt();
        if (crc != MaskedCrc32c.masked(encoded, 0, 188))
            throw new RpcProtocolException("HELLO checksum mismatch");
        RpcHelloV1 hello;
        try {
            hello =
                    new RpcHelloV1(
                            role, cluster, node, session, nonce, frame, message, streams, window,
                            idle, timeout, major, minor, patch, java);
        } catch (IllegalArgumentException invalid) {
            throw new RpcProtocolException(invalid.getMessage());
        }
        if (!MessageDigest.isEqual(fingerprint, compatibilityFingerprint()))
            throw new RpcProtocolException("HELLO compatibility mismatch");
        return hello;
    }
    ...
    public static byte[] compatibilityFingerprint() {
        try {
            return MessageDigest.getInstance("SHA-256")
                    .digest(
                            ("AETHER-RPC-COMPAT-V1|1.0|64|1,2,3,4,5,6,7,8,9|"
                                            + "BEGIN=1,END=2|status-v1|hello-v1|crc32c-mask-v1")
    ...
    private static void putUuid(ByteBuffer bytes, UUID value) {
        bytes.putLong(value.getMostSignificantBits()).putLong(value.getLeastSignificantBits());
    }
