 ...
 */
public record RpcFrameHeaderV1(
        RpcFrameType type,
        int flags,
        long streamId,
        int code,
    ...
    public RpcFrameHeaderV1 {
        ...
        if ((flags & ~(BEGIN | END)) != 0
                || streamId < 0
                || payloadLength < 0
                || payloadLength > MAX_FRAME_PAYLOAD
                || messageLength < 0
                || messageLength > MAX_MESSAGE_BYTES
                || fragmentOffset < 0
                || timeoutMillis < 0
                || creditDelta < 0) throw new IllegalArgumentException("invalid RPC frame fields");
        if (type.control() != (streamId == 0) && type != RpcFrameType.CANCEL)
            throw new IllegalArgumentException("invalid stream ID for frame type");
        if (type == RpcFrameType.CANCEL && streamId == 0)
            throw new IllegalArgumentException("CANCEL requires a stream");
        if ((flags & BEGIN) == 0 && messageLength != 0)
            throw new IllegalArgumentException("only BEGIN declares message length");
        if (type != RpcFrameType.REQUEST && timeoutMillis != 0)
            throw new IllegalArgumentException("timeout is request-only");
        if (type != RpcFrameType.WINDOW_UPDATE && creditDelta != 0)
            throw new IllegalArgumentException("credit is WINDOW_UPDATE-only");
        if (type == RpcFrameType.WINDOW_UPDATE && creditDelta == 0)
            throw new IllegalArgumentException("WINDOW_UPDATE credit must be positive");
        if (type.control()
                && type != RpcFrameType.HELLO
                && type != RpcFrameType.GOAWAY
                && type != RpcFrameType.CONNECTION_ERROR
                && !invocationId.equals(ZERO_INVOCATION))
     ...
     */
    public boolean beginsMessage() {
        return (flags & BEGIN) != 0;
    }
