...
public enum RpcFrameType {
    /** Connection capability and identity handshake. */
    HELLO(1, true),
    /** Application request fragment. */
    REQUEST(2, false),
    /** Application response fragment. */
    RESPONSE(3, false),
    /** Stream cancellation notification. */
    CANCEL(4, false),
    /** Connection receive-credit replenishment. */
    WINDOW_UPDATE(5, true),
    /** Keepalive or reachability probe. */
    PING(6, true),
    /** Reply to a ping. */
    PONG(7, true),
    /** Graceful connection-drain notification. */
    GOAWAY(8, true),
    /** Terminal connection-level protocol failure. */
    CONNECTION_ERROR(9, true);
    private final int code;
    private final boolean control;

    RpcFrameType(int code, boolean control) {
        this.code = code;
