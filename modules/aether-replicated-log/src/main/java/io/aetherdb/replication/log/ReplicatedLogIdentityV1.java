...
public record ReplicatedLogIdentityV1(UUID clusterId, UUID nodeId, long creationEpochMillis) {
    ...
    public ReplicatedLogIdentityV1 {
        if (isZero(clusterId) || isZero(nodeId) || creationEpochMillis < 0)
            throw new IllegalArgumentException("invalid replicated-log identity");
    }
    ...
    public byte[] encode() {
        ...
        bytes.position(252).putInt(MaskedCrc32c.masked(result, 0, 252));
        return result;
    }
    ...
    public static ReplicatedLogIdentityV1 decode(byte[] encoded) {
        ...
        for (int index = 136; index < 252; index++)
            if (encoded[index] != 0)
                throw new IllegalArgumentException("nonzero RLOG-IDENTITY reserved byte");
        int crc = ByteBuffer.wrap(encoded, 252, 4).order(ByteOrder.LITTLE_ENDIAN).getInt();
        if (crc != MaskedCrc32c.masked(encoded, 0, 252)
                || !MessageDigest.isEqual(fingerprint, compatibilityFingerprint()))
    ...
    public static byte[] compatibilityFingerprint() {
        ...
        vector.put("AETHER-RLOG-COMPAT-V1".getBytes(StandardCharsets.US_ASCII))
                .putLong(1)
                .putLong(134_217_728L)
                .putLong(268_435_456L)
                .putLong(67_108_864L)
                .putInt(192)
                .putInt(4096)
                .putInt(8)
