...
public final class ReplicatedLogEntryCodecV1 {
    ...
    public static ReplicatedLogEntry create(
            ReplicatedEntryType type,
        ...
        ReplicatedLogEntry entry =
                new ReplicatedLogEntry(
                        ...
                        entryHash,
                        payloadCopy);
        validateCommandConsistency(entry);
        return entry;
    }
    ...
    public static byte[] encode(ReplicatedLogEntry entry) {
        ...
        validateCommandConsistency(entry);
        byte[] payload = entry.payload(), payloadHash = sha256(payload);
        byte[] expectedHash =
                entryHash(
                        entry.type(),
                        entry.payloadFormatVersion(),
                        entry.index(),
                        entry.term(),
                        entry.commandId(),
                        entry.sequenceStart(),
        ...
        if (!MessageDigest.isEqual(expectedHash, entry.entryHash()))
            throw new IllegalArgumentException("entry hash mismatch");
        ...
        bytes.put("AERE".getBytes(StandardCharsets.US_ASCII))
                .putShort((short) 1)
                .putShort((short) ReplicatedLogFormatV1.ENTRY_HEADER_BYTES)
                .putInt(length)
                .put((byte) entry.type().code())
                .put((byte) 0)
                .putShort((short) entry.payloadFormatVersion())
                .putLong(entry.index())
                .putLong(entry.term())
                .putLong(entry.commandId().getMostSignificantBits())
                .putLong(entry.commandId().getLeastSignificantBits())
                .putLong(entry.sequenceStart())
                .putLong(entry.stateSequenceAfter())
                .putInt(payload.length)
                .putInt(0)
                .put(payloadHash)
                .put(entry.previousEntryHash())
                .put(entry.entryHash());
        bytes.putInt(MaskedCrc32c.masked(result, 0, 168));
        bytes.position(ReplicatedLogFormatV1.ENTRY_HEADER_BYTES).put(payload);
        int trailer = length - ReplicatedLogFormatV1.ENTRY_TRAILER_BYTES;
        byte[] magic = "AEND".getBytes(StandardCharsets.US_ASCII);
        System.arraycopy(magic, 0, result, trailer + 4, 4);
        byte[] coverage = new byte[trailer + 4];
        System.arraycopy(result, 0, coverage, 0, trailer);
        System.arraycopy(magic, 0, coverage, trailer, 4);
        ByteBuffer.wrap(result)
                .order(ByteOrder.LITTLE_ENDIAN)
                .putInt(trailer, MaskedCrc32c.masked(coverage, 0, coverage.length));
        return result;
    }
    ...
    public static ReplicatedLogEntry decode(byte[] record) {
        if (record == null
                || record.length < 200
                || record.length > ReplicatedLogFormatV1.MAXIMUM_ENTRY_RECORD_BYTES
        ...
        bytes.get(magic);
        if (!Arrays.equals(magic, "AERE".getBytes(StandardCharsets.US_ASCII))
                || bytes.getShort() != 1
                || bytes.getShort() != ReplicatedLogFormatV1.ENTRY_HEADER_BYTES
                || bytes.getInt() != record.length) {
        ...
        int payloadVersion = Short.toUnsignedInt(bytes.getShort());
        long index = bytes.getLong(), term = bytes.getLong();
        UUID command = new UUID(bytes.getLong(), bytes.getLong());
        long sequenceStart = bytes.getLong(), stateAfter = bytes.getLong();
        int payloadLength = bytes.getInt();
        if (bytes.getInt() != 0) throw new IllegalArgumentException("nonzero entry reserved field");
        byte[] payloadHash = new byte[32],
                previousHash = new byte[32],
                storedEntryHash = new byte[32];
        bytes.get(payloadHash).get(previousHash).get(storedEntryHash);
        int storedHeaderCrc = bytes.getInt();
        for (int offset = 172; offset < 192; offset++)
            if (record[offset] != 0)
                throw new IllegalArgumentException("nonzero entry reserved byte");
        if (storedHeaderCrc != MaskedCrc32c.masked(record, 0, 168))
            throw new IllegalArgumentException("entry header checksum mismatch");
        if (payloadLength < 0
                || ReplicatedLogFormatV1.recordLength(payloadLength) != record.length) {
        ...
        for (int offset = payloadEnd; offset < trailer; offset++)
            if (record[offset] != 0)
                throw new IllegalArgumentException("nonzero entry alignment padding");
        if (record[trailer + 4] != 'A'
                || record[trailer + 5] != 'E'
                || record[trailer + 6] != 'N'
                || record[trailer + 7] != 'D') {
        ...
        byte[] coverage = new byte[trailer + 4];
        System.arraycopy(record, 0, coverage, 0, trailer);
        ...
        byte[] payload =
                Arrays.copyOfRange(record, ReplicatedLogFormatV1.ENTRY_HEADER_BYTES, payloadEnd);
        if (!MessageDigest.isEqual(payloadHash, sha256(payload)))
            throw new IllegalArgumentException("entry payload hash mismatch");
        byte[] calculatedEntryHash =
                entryHash(
                        type,
                        payloadVersion,
                        index,
                        ...
                        payloadHash,
                        previousHash);
        if (!MessageDigest.isEqual(storedEntryHash, calculatedEntryHash))
            throw new IllegalArgumentException("entry hash-chain mismatch");
        ReplicatedLogEntry entry =
                new ReplicatedLogEntry(
                        type,
                        payloadVersion,
    ...
    private static byte[] entryHash(
        ...
        if (type == null
            ...
            throw new IllegalArgumentException("invalid entry hash input");
        byte[] prefix = "AETHER-RLOG-ENTRY-V1".getBytes(StandardCharsets.US_ASCII);
        ByteBuffer bytes =
                ByteBuffer.allocate(prefix.length + 32 + 1 + 1 + 2 + 6 * 8 + 4 + 32)
                        .order(ByteOrder.LITTLE_ENDIAN);
        bytes.put(prefix)
                .put(previousHash)
                .put((byte) type.code())
                .put((byte) 0)
                .putShort((short) payloadVersion)
                .putLong(index)
                .putLong(term)
                .putLong(commandId.getMostSignificantBits())
                .putLong(commandId.getLeastSignificantBits())
                .putLong(sequenceStart)
                .putLong(stateAfter)
    ...
    private static void validateCommandConsistency(ReplicatedLogEntry entry) {
        ...
        if (!command.commandId().equals(entry.commandId())
                || command.sequences().first() != entry.sequenceStart()
