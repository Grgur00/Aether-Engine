...
public final class ReplicatedLogFormatV1 {
    /** Segment header region and meaningful header sizes. */
    public static final int SEGMENT_HEADER_REGION_BYTES = 4096, SEGMENT_HEADER_BYTES = 192;

    /** Entry header and trailer sizes. */
    public static final int ENTRY_HEADER_BYTES = 192, ENTRY_TRAILER_BYTES = 8;

    /** Alignment and persistent segment limits. */
    public static final int ALIGNMENT_BYTES = 8;
    ...
    /** Maximum complete aligned entry record bytes. */
    public static final int MAXIMUM_ENTRY_RECORD_BYTES = 67_108_864;
    ...
    public static int recordLength(int payloadBytes) {
        ...
        int unaligned =
                Math.addExact(Math.addExact(ENTRY_HEADER_BYTES, payloadBytes), ENTRY_TRAILER_BYTES);
        int padding = (ALIGNMENT_BYTES - unaligned % ALIGNMENT_BYTES) % ALIGNMENT_BYTES;
        int result = Math.addExact(unaligned, padding);
        if (result > MAXIMUM_ENTRY_RECORD_BYTES)
            throw new IllegalArgumentException("replicated entry record too large");
        return result;
    }
}
