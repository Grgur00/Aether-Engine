...
final class RecordDialog {
    ...
    static Optional<RecordInput> show(
        ...
        constraints.gridx = 0;
        constraints.gridy = 0;
        form.add(new JLabel("Key"), constraints);
        constraints.gridx = 1;
        constraints.weightx = 1;
        constraints.fill = GridBagConstraints.HORIZONTAL;
        form.add(keyField, constraints);
        constraints.gridx = 0;
        constraints.gridy = 1;
        constraints.weightx = 0;
        constraints.fill = GridBagConstraints.NONE;
        form.add(
                new JLabel(valueEditor instanceof StructuredEditor ? "Fields" : "Value"),
                constraints);
        constraints.gridx = 1;
        constraints.weightx = 1;
        ...
        if (keyField.getText().isEmpty()) {
            JOptionPane.showMessageDialog(
                    parent,
                    "Key must not be empty in the text workbench.",
                    "Invalid key",
    ...
    private static final class StructuredEditor implements ValueEditor {
        ...
        static Optional<StructuredEditor> create(String encoded) {
            ...
            for (String line : encoded.split("\\R", -1)) {
                ...
                int id;
                try {
                    id = Integer.parseInt(line.substring(0, colon).strip());
                ...
                String type =
                        (nameSeparator < 0 ? metadata : metadata.substring(nameSeparator + 1))
                                .strip();
                if (!isFieldType(type)) return Optional.empty();
                String value;
                try {
                    value = UniversalTypedValue.unescapeText(line.substring(equals + 1));
        ...
        public String value() {
            ...
            for (FieldInput field : fields) {
                ...
                encoded.append(field.type())
                        .append('=')
                        .append(UniversalTypedValue.escapeText(field.input().getText()))
                        .append('\n');
            }
