...
final class UniversalTypedValue {
    ...
    static boolean isTyped(byte[] value) {
        return value != null && value.length >= TypedValueEnvelope.HEADER_BYTES
                && value[0] == 'A' && value[1] == 'E' && value[2] == 'T' && value[3] == 'V';
    }
    ...
    static String display(byte[] value, byte[] descriptor) {
        ...
        CanonicalRecordReader reader = new CanonicalRecordReader(
                envelope.payload(), MAXIMUM_RECORD_BYTES);
        while (reader.next()) {
            ...
            output.append(typeName(reader.wireType())).append('=')
                    .append(escapeText(displayField(reader))).append('\n');
        }
    ...
    static byte[] reencode(byte[] original, String edited) {
        ...
        byte[] payload = isAer1(envelope.payload())
                ? encodeRecord(edited)
    ...
    private static byte[] encodeRecord(String edited) {
        ...
        for (String line : edited.split("\\R", -1)) {
            ...
            int fieldId;
            try { fieldId = Integer.parseInt(line.substring(0, colon).strip()); }
            catch (NumberFormatException failure) {
                throw new IllegalArgumentException("invalid field ID: " + line.substring(0, colon), failure);
            }
    ...
    private static Map<Integer, String> descriptorFieldNames(byte[] descriptor) {
        ...
        for (String line : text.split("\\n")) {
            ...
            try { names.put(Integer.parseInt(parts[0]), parts[1]); }
            catch (NumberFormatException ignored) { /* malformed descriptor is not trusted */ }
        }
    ...
    private static byte[] encodeField(String type, String value) {
        try {
            return switch (type) {
                ...
                default -> {
                    if (!type.startsWith("wire-")) throw new IllegalArgumentException("unknown type: " + type);
    ...
    private static int wireType(String type) {
        return switch (type) {
            ...
            default -> {
                if (!type.startsWith("wire-")) throw new IllegalArgumentException("unknown type: " + type);
                try { yield Integer.parseInt(type.substring(5)); }
                catch (NumberFormatException failure) {
    ...
    private static byte[] encodeScalar(byte[] original, String edited) {
        if (isDisplayableUtf8(original)) return unescapeText(edited).getBytes(StandardCharsets.UTF_8);
        if (edited.startsWith("hex:")) {
            try { return HEX.parseHex(edited.substring(4).strip()); }
            catch (IllegalArgumentException failure) {
        ...
        for (int index = 0; index < value.length(); index++) {
            ...
            if (escaped) {
                decoded.append(switch (current) {
                    ...
                    case 'r' -> '\r';
                    case '\\' -> '\\';
                    default -> throw new IllegalArgumentException("unsupported escape: \\" + current);
                });
                escaped = false;
            }
    ...
    private static boolean isAer1(byte[] payload) {
        return payload.length >= CanonicalRecordWriter.HEADER_BYTES
                && payload[0] == 'A' && payload[1] == 'E'
                && payload[2] == 'R' && payload[3] == '1';
    }
    ...
    private record Envelope(byte[] header, byte[] payload) {
        static Envelope read(byte[] value) {
            ...
            if (flags != 0 || reserved != 0 || payloadLength < 0 || payloadLength != input.remaining()) {
