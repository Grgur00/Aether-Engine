...
public final class AetherWorkbench {
                ...
                () -> {
                    ...
                    try {
                        if (arguments.length == 0) {
                            new AetherWorkbench(
                                            Aether.openInMemory(), true, false, "in-memory session")
                                    .frame.setVisible(true);
                        } else {
                            Path directory = Path.of(arguments[0]).toAbsolutePath().normalize();
                            new AetherWorkbench(
                                            Aether.open(directory),
                                            true,
                                            true,
                                            directory.toString())
                                    .frame.setVisible(true);
    ...
    /**
     * Opens the workbench on an application's existing live database without taking ownership of
     * it.
     *
    ...
    public static void open(io.aetherdb.api.AetherDatabase database) {
        ...
        SwingUtilities.invokeLater(
                () ->
                        new AetherWorkbench(database, false, false, "attached application session")
                                .frame.setVisible(true));
    }
    ...
    private void build() {
        ...
        heading.setBorder(BorderFactory.createEmptyBorder(16, 18, 12, 18));
        JLabel title = new JLabel("Aether Data Explorer");
        title.setFont(title.getFont().deriveFont(Font.BOLD, 22f));
        JLabel notice =
                new JLabel(
                        persistent
                                ? "Persistent database • schema-aware editing • "
                                        + sessionDescription
                                : "In-memory session • data is discarded when this window "
                                        + "closes");
        notice.setForeground(new Color(150, 75, 0));
        heading.add(title, BorderLayout.NORTH);
        heading.add(notice, BorderLayout.SOUTH);
        ...
        edit.addActionListener(event -> editRecord());
        delete.addActionListener(event -> deleteRecord());
        refresh.addActionListener(event -> refresh("Refreshed"));
        add.setEnabled(true);
        ...
        for (int column = 0; column < table.getColumnCount(); column++)
            table.getColumnModel().getColumn(column).setCellRenderer(groupedRenderer);
        ...
        scroll.setBorder(BorderFactory.createEmptyBorder(0, 18, 0, 18));
    ...
    private void addRecord() {
        ...
        RecordDialog.show(frame, "Add record", "", "")
                .ifPresent(
                        input -> {
                            if (workspace.contains(input.key())) {
                                showError(
                                        "That key already exists. Select it and choose Edit"
                                                + " selected.");
                                return;
                            }
                            workspace.put(input.key(), input.value());
                            refresh("Added “" + input.key() + "”");
                        });
    }
    ...
    private void editRecord() {
        ...
        RecordDialog.show(
                        ...
                        selected.key(),
                        selected.value(),
                        workspace.keyEditable(selected.key()))
                .ifPresent(
                        input -> {
                            try {
                                workspace.edit(selected.key(), input.key(), input.value());
                                refresh("Updated “" + input.key() + "”");
                            } catch (IllegalArgumentException failure) {
                                showError(failure.getMessage());
                            }
                        });
    ...
    private void deleteRecord() {
        ...
        if (choice == JOptionPane.OK_OPTION) {
            workspace.delete(selected.key());
            refresh("Deleted “" + selected.key() + "”");
        }
    }
    ...
    private void refresh(String message) {
        ...
        String recordCount = workspace.size() + " record(s)";
        status.setText(String.join(" • ", message, recordCount, sessionDescription));
    }

    private void showError(String message) {
        JOptionPane.showMessageDialog(
                frame, message, "Aether Workbench", JOptionPane.WARNING_MESSAGE);
    }
        ...
        public Component getTableCellRendererComponent(
            ...
            boolean startsGroup =
                    viewRow == 0
                            || !row.group()
                                    .equals(
                                            model.row(source.convertRowIndexToModel(viewRow - 1))
                                                    .group());
            Object displayValue = viewColumn == 0 && !startsGroup ? "" : value;
            Component component =
                    super.getTableCellRendererComponent(
                            source, displayValue, selected, focused, viewRow, viewColumn);
            if (!selected)
                component.setBackground(
                        (row.group().hashCode() & 1) == 0
                                ? alternateGroup
            ...
            setFont(
                    viewColumn == 0 && startsGroup
                            ? source.getFont().deriveFont(Font.BOLD)
                            : source.getFont());
            return component;
        }
    }
