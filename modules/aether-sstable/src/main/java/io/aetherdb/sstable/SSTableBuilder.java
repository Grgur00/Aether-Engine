...
public final class SSTableBuilder {
    ...
    public void add(InternalKey key, byte[] value) {
        ...
        if (key.type() == 2 && value.length != 0)
            throw new IllegalArgumentException("tombstone value must be empty");
        if (!entries.isEmpty()
                && INTERNAL_ORDER.compare(entries.get(entries.size() - 1).key, encoded) >= 0) {
    ...
    public TableFileMetadata finish() throws IOException {
        ...
        BlockHandle prospectiveProperties =
                new BlockHandle(propertiesOffset, propertiesPhysicalLength);
        ...
        SSTableHeaderV1 header =
                new SSTableHeaderV1(
                        ...
                        databaseId,
                        entries.size(),
                        metrics.smallestSequence,
                        metrics.largestSequence,
                        dataBlocks.size(),
                        creationEpochMillis,
                        table.length);
    ...
    public void abandon() {
        if (state == State.OPEN) state = State.ABANDONED;
    }
    ...
    private byte[] index(List<BlockDescription> blocks) {
        List<RestartBlock.Entry> indexEntries =
                blocks.stream()
                        .map(
                                block ->
                                        new RestartBlock.Entry(
                                                block.largestKey, block.handle.encode()))
                        .toList();
    ...
    private static byte[] metaindex(BlockHandle filter, BlockHandle properties) {
        return RestartBlock.encode(
                List.of(
                        new RestartBlock.Entry(ascii("aether.filter.bloom.v1"), filter.encode()),
                        new RestartBlock.Entry(ascii("aether.properties.v1"), properties.encode())),
                1);
    }
    ...
    private byte[] properties(Metrics metrics, long fileSize) {
        ...
        values.put("aether.file.number", littleLong(fileNumber));
        values.put("aether.file.size", littleLong(fileSize));
        ...
        List<RestartBlock.Entry> encoded =
                values.entrySet().stream()
                        .map(
                                entry ->
                                        new RestartBlock.Entry(
                                                ascii(entry.getKey()), entry.getValue()))
                        .toList();
    ...
    private TableFileMetadata metadata(Metrics metrics, int blocks, long size) {
        return new TableFileMetadata(
                ...
                metrics.largestSequence,
                metrics.rawKeyBytes,
                metrics.rawValueBytes);
    }

    private void requireOpen() {
        if (state != State.OPEN) throw new IllegalStateException("builder is " + state);
    }
    ...
    private static byte[] littleLong(long value) {
