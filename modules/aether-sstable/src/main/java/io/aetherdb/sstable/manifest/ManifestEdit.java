...
public record ManifestEdit(
        ...
        long persistedSequenceWatermark,
        long minimumWalFileNumber,
        List<ManifestFileMetadata> additions,
        List<ManifestDeletion> deletions) {
    /** Durable edit kind. */
    public enum Kind {
        /** Complete live-file snapshot. */
        SNAPSHOT,
        /** Incremental change from the preceding version. */
        DELTA
    }
    ...
    public ManifestEdit {
        if (kind == null
                ...
                || persistedSequenceWatermark < 0
                || persistedSequenceWatermark > lastAssignedSequence
                || minimumWalFileNumber < 0
                || kind == Kind.DELTA && minimumWalFileNumber == 0
                || additions == null
                || deletions == null
