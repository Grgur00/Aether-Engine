...
public final class Version {
    ...
    private static final Comparator<ManifestFileMetadata> LEVEL_ORDER =
            (left, right) ->
                    Arrays.compareUnsigned(left.smallestUserKey(), right.smallestUserKey());
    ...
    public Version apply(ManifestEdit delta) {
        if (delta == null || delta.kind() != ManifestEdit.Kind.DELTA)
    ...
    private static Version applyTo(Version base, ManifestEdit edit, long generation) {
        ...
        if (base != null)
            for (List<ManifestFileMetadata> level : base.levels)
                for (ManifestFileMetadata file : level) files.put(file.fileNumber(), file);
        for (ManifestDeletion deletion : edit.deletions()) {
            ManifestFileMetadata removed = files.remove(deletion.fileNumber());
            if (removed == null || removed.level() != deletion.level())
                throw new IllegalArgumentException("deletion does not name a live file");
        }
        for (ManifestFileMetadata addition : edit.additions()) {
            if (files.putIfAbsent(addition.fileNumber(), addition) != null)
                throw new IllegalArgumentException("addition reuses a live file number");
        }
        if (base != null
                && (edit.editNumber() != base.manifestEditNumber + 1
                        || edit.nextFileNumber() < base.nextFileNumber
        ...
        for (int number = 0; number < 7; number++) {
            ...
            List<ManifestFileMetadata> level =
                    files.values().stream()
                            .filter(file -> file.level() == levelNumber)
                            .sorted(number == 0 ? L0_ORDER : LEVEL_ORDER)
                            .toList();
    ...
    private static void validateNonOverlapping(List<ManifestFileMetadata> level) {
        for (int index = 1; index < level.size(); index++) {
            if (Arrays.compareUnsigned(
                            level.get(index - 1).largestUserKey(),
                            level.get(index).smallestUserKey())
                    >= 0) {
     ...
     */
    public List<ManifestFileMetadata> allFiles() {
        return levels.stream().flatMap(List::stream).toList();
    }

    /**
     * Returns the first unallocated file number.
     *
     * @return positive allocation counter
     */
     ...
     *
     * @return nonnegative sequence
     */
    public long lastAssignedSequence() {
     ...
     *
     * @return nonnegative flush watermark
     */
    public long persistedSequenceWatermark() {
        return persistedSequenceWatermark;
    }

    /**
     * Returns the oldest WAL still needed for recovery.
     *
     * @return positive WAL file number
     */
    public long minimumWalFileNumber() {
        return minimumWalFileNumber;
    }

    /**
     * Returns the manifest generation containing this version.
     *
     * @return positive generation
     */
     ...
     *
     * @return positive contiguous edit number
     */
    public long manifestEditNumber() {
        return manifestEditNumber;
    }
}
