...
public record ManifestHeaderV1(
    ...
    public byte[] encodeRegion() {
        byte[] region = new byte[HEADER_REGION_BYTES];
        ByteBuffer bytes = little(region);
        bytes.put("AETHMAN1".getBytes(StandardCharsets.US_ASCII))
                .putShort((short) 1)
                .putShort((short) HEADER_BYTES)
                .putInt(0)
                .putLong(databaseId.getMostSignificantBits())
                .putLong(databaseId.getLeastSignificantBits())
                .putLong(manifestFileNumber)
                .putInt(1)
                .putInt(7)
                .putLong(creationEpochMillis)
                .putLong(initialNextFileNumber)
                .putLong(initialLastSequence);
        bytes.putInt(92, MaskedCrc32c.masked(region, 0, 92));
        return region;
    }
    ...
    public static ManifestHeaderV1 decodeRegion(byte[] region) {
        if (region == null || region.length != HEADER_REGION_BYTES)
            throw corrupt("invalid manifest header region");
        if (little(region).getInt(92) != MaskedCrc32c.masked(region, 0, 92))
            throw corrupt("manifest header checksum mismatch");
        ByteBuffer bytes = little(region);
        ...
        if (bytes.getInt() != 1 || bytes.getInt() != 7)
            throw corrupt("unsupported manifest comparator or level count");
        long created = bytes.getLong(), next = bytes.getLong(), last = bytes.getLong();
        for (int index = 72; index < 92; index++)
            if (region[index] != 0) throw corrupt("nonzero manifest header reserved byte");
        for (int index = HEADER_BYTES; index < region.length; index++)
            if (region[index] != 0) throw corrupt("nonzero manifest region padding");
        try {
            return new ManifestHeaderV1(databaseId, fileNumber, created, next, last);
        } catch (IllegalArgumentException failure) {
            throw new ManifestCorruptionException("invalid manifest header fields", failure);
        }
    }
