...
public record SSTableFooterV1(
        BlockHandle metaindex, BlockHandle index, BlockHandle filter, BlockHandle properties,
        long fileNumber, long fileSize, UUID databaseId) {
    /** Fixed footer size. */ public static final int FOOTER_BYTES = 128;

    /** Validates required footer values. */
    public SSTableFooterV1 {
        if (metaindex == null || index == null || filter == null || properties == null
                || fileNumber <= 0 || fileSize < SSTableHeaderV1.HEADER_REGION_BYTES + FOOTER_BYTES
    ...
    public byte[] encode() {
        ...
        bytes.put("AETHFTR1".getBytes(StandardCharsets.US_ASCII)).putShort((short) 1)
                .putShort((short) FOOTER_BYTES).putInt(0).put(metaindex.encode()).put(index.encode())
                .put(filter.encode()).put(properties.encode()).putLong(fileNumber).putLong(fileSize)
                .putLong(databaseId.getMostSignificantBits()).putLong(databaseId.getLeastSignificantBits());
    ...
    public static SSTableFooterV1 decode(byte[] encoded) {
        if (encoded == null || encoded.length != FOOTER_BYTES) throw corrupt("invalid footer length");
        ...
        if (!Arrays.equals(magic, "AETHFTR1".getBytes(StandardCharsets.US_ASCII))
                || Short.toUnsignedInt(bytes.getShort()) != 1 || Short.toUnsignedInt(bytes.getShort()) != FOOTER_BYTES
                || bytes.getInt() != 0) throw corrupt("invalid footer prefix");
        BlockHandle metaindex = handle(bytes), index = handle(bytes), filter = handle(bytes), properties = handle(bytes);
        long fileNumber = bytes.getLong(), fileSize = bytes.getLong();
        UUID databaseId = new UUID(bytes.getLong(), bytes.getLong());
        for (int position = 112; position < 124; position++) if (encoded[position] != 0) throw corrupt("nonzero footer reserved byte");
        try { return new SSTableFooterV1(metaindex, index, filter, properties, fileNumber, fileSize, databaseId); }
        catch (IllegalArgumentException failure) { throw corrupt("invalid footer fields"); }
    }
    ...
    public void validateHandles() {
        ...
        for (int left = 0; left < handles.length; left++) for (int right = left + 1; right < handles.length; right++) {
            if (overlap(handles[left], handles[right])) throw corrupt("mandatory metadata blocks overlap");
        }
    }

    private static BlockHandle handle(ByteBuffer bytes) {
        byte[] encoded = new byte[BlockHandle.ENCODED_BYTES]; bytes.get(encoded); return BlockHandle.decode(encoded);
    }
