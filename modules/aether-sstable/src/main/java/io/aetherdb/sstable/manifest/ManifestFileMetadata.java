...
public record ManifestFileMetadata(
        long fileNumber, int level, long fileSize, long entryCount,
        long smallestSequence, long largestSequence,
        byte[] smallestInternalKey, byte[] largestInternalKey) {
    /** Validates persisted fields and takes defensive key copies. */
    public ManifestFileMetadata {
        if (fileNumber <= 0 || level < 0 || level > 6 || fileSize <= 0 || entryCount <= 0
                || smallestSequence <= 0 || largestSequence < smallestSequence
                || smallestInternalKey == null || largestInternalKey == null) {
        ...
        if (smallest.compareTo(largest) > 0
                || smallest.sequence() < smallestSequence || smallest.sequence() > largestSequence
                || largest.sequence() < smallestSequence || largest.sequence() > largestSequence) {
    ...
    public boolean contentEquals(ManifestFileMetadata other) {
        return other != null && fileNumber == other.fileNumber && level == other.level
                && fileSize == other.fileSize && entryCount == other.entryCount
                && smallestSequence == other.smallestSequence && largestSequence == other.largestSequence
