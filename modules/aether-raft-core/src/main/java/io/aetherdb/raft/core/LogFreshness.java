...
public final class LogFreshness {
     ...
     */
    public static Decision compare(
            long candidateTerm,
            long candidateIndex,
            byte[] candidateHash,
            long localTerm,
        ...
        if (candidateTerm != localTerm)
            return candidateTerm > localTerm ? Decision.UP_TO_DATE : Decision.STALE;
        if (candidateIndex != localIndex)
            return candidateIndex > localIndex ? Decision.UP_TO_DATE : Decision.STALE;
        return MessageDigest.isEqual(candidateHash, localHash)
                ? Decision.UP_TO_DATE
                : Decision.DIVERGED;
    }
    ...
    public enum Decision {
        /** Candidate log is at least as current and has a matching integrity anchor. */
        UP_TO_DATE,
        /** Candidate term or index is behind the receiver. */
        STALE,
        /** Coordinates match but integrity hashes differ. */
        DIVERGED
    }
}
