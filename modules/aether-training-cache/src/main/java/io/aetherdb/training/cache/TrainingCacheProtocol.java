...
final class TrainingCacheProtocol {
    ...
    static void serve(InputStream rawInput, OutputStream rawOutput, TrainingCache cache, Semaphore permits,
        ...
        metrics.connection();
        try (DataInputStream input = new DataInputStream(rawInput); DataOutputStream output = new DataOutputStream(rawOutput)) {
            while (true) {
                int frameBytes;
                ...
                if (frame.length != frameBytes) throw new EOFException();
                if (!permits.tryAcquire()) { writeResponse(output, ERROR, new byte[0]); continue; }
                try {
                    ByteBuffer request = ByteBuffer.wrap(frame);
                    if (request.get() != VERSION) throw new IOException("unsupported protocol version");
                    int operation = request.get();
                    ...
                    if (operation == CONTAINS_MANY) {
                        ...
                        for (int index = 0; index < count; index++)
                            result.put((byte) (cache.containsKey(readKey(request)) ? 1 : 0));
                        ...
                        for (int index = 0; index < count; index++) {
                            ...
                            SegmentReference reference = cache.getRef(key);
                        ...
                        BatchValueResult batch = cache.getManyValues(keys);
                        java.io.ByteArrayOutputStream packed = new java.io.ByteArrayOutputStream();
                        java.io.DataOutputStream result = new java.io.DataOutputStream(packed);
                        result.writeInt(count);
                        for (BatchValueResult.ValueStatus status : batch.statuses())
                            result.writeByte(status == BatchValueResult.ValueStatus.MISS ? 0 : 1);
                        for (int offset : batch.offsets()) result.writeInt(offset);
                        for (int length : batch.lengths()) result.writeInt(length);
                        java.nio.ByteBuffer payload = batch.payload().duplicate();
                        byte[] bytes = new byte[payload.remaining()];
                        payload.get(bytes);
                        result.write(bytes);
                        writeResponse(output, HIT, packed.toByteArray());
                    } else {
                    CacheKey key = readKey(request);
                    if (operation == GET_REF) {
                        ...
                        cache.put(key, readValue(request));
                        writeResponse(output, HIT, new byte[0]);
