...
public record WriteOptions(
        DurabilityMode durabilityMode, Duration admissionTimeout, boolean failFastOnBackpressure) {
    ...
    public WriteOptions {
        ...
        if (admissionTimeout.isNegative())
            throw new IllegalArgumentException("admission timeout must be non-negative");
    }
    ...
    public static WriteOptions defaults() {
        return new WriteOptions(DurabilityMode.GROUP_SYNC, Duration.ofSeconds(30), false);
    }
}
