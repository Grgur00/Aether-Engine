 ...
 */
public record CollectionDefinition<K, V>(CollectionId id, String name, KeyCodec<K> keyCodec,
                                          ValueCodec<V> valueCodec, Set<CollectionCapability> capabilities) {
    /** Validates codec bounds and normalizes the capability set. */
    public CollectionDefinition {
        if (id == null || name == null || name.isBlank()
                || name.getBytes(StandardCharsets.UTF_8).length > 128
                || keyCodec == null || valueCodec == null) {
            throw new IllegalArgumentException("invalid collection definition");
        }
        if (keyCodec.encodingVersion() < 1 || keyCodec.encodingVersion() > 65535
                || keyCodec.maximumEncodedSize() < 0 || keyCodec.maximumEncodedSize() > 65517) {
            throw new IllegalArgumentException("invalid key codec bounds");
        }
        if (keyCodec.fingerprint() == null || keyCodec.fingerprint().length != 32
                || valueCodec.fingerprint() == null || valueCodec.fingerprint().length != 32) {
    ...
    public static <K, V> CollectionDefinition<K, V> of(
        ...
        return new CollectionDefinition<>(id, name, keyCodec, valueCodec,
                Set.of(CollectionCapability.POINT_READ, CollectionCapability.POINT_WRITE));
    }
}
