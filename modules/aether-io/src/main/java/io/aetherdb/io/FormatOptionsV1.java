...
public record FormatOptionsV1(UUID databaseId, long creationEpochMillis) {
    ...
    public byte[] encode() {
        ...
        bytes.putLong(databaseId.getMostSignificantBits()).putLong(databaseId.getLeastSignificantBits());
    ...
    public static FormatOptionsV1 decode(byte[] encoded) {
        ...
        if (!Arrays.equals(magic, MAGIC) || Short.toUnsignedInt(bytes.getShort()) != 1
                || Short.toUnsignedInt(bytes.getShort()) != HEADER_LENGTH || bytes.getInt() != ENCODED_LENGTH)
        ...
        byte[] storedFingerprint = new byte[32]; bytes.position(128).get(storedFingerprint);
        for (int index = 160; index < 508; index++) if (encoded[index] != 0) throw new IllegalArgumentException("nonzero FORMAT-OPTIONS reserved byte");
        for (int index = 512; index < encoded.length; index++) if (encoded[index] != 0) throw new IllegalArgumentException("nonzero FORMAT-OPTIONS tail byte");
        int storedCrc = ByteBuffer.wrap(encoded, 508, 4).order(ByteOrder.LITTLE_ENDIAN).getInt();
        if (storedCrc != MaskedCrc32c.masked(encoded, 0, 508)) throw new IllegalArgumentException("FORMAT-OPTIONS checksum mismatch");
    ...
    private static void verifyFormatVector(ByteBuffer bytes) {
        ByteBuffer expected = ByteBuffer.allocate(88).order(ByteOrder.LITTLE_ENDIAN); putFormatVector(expected);
        byte[] actual = new byte[88]; bytes.position(32).get(actual);
        if (!Arrays.equals(actual, expected.array())) throw new IllegalArgumentException("unsupported FORMAT-OPTIONS vector");
    }
}
