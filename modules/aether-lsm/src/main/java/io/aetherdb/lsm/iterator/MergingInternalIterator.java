...
public final class MergingInternalIterator implements InternalIterator {
     ...
     */
    public MergingInternalIterator(List<? extends InternalIterator> sources) {
    ...
    public boolean next() {
        ...
        if (!initialized) {
            initialized = true;
            for (InternalIterator source : sources)
                if (source.next()) heap.add(new Head(source, source.current()));
        ...
        for (Head head : heap)
            if (head != first && first.entry().sameIdentity(head.entry()))
                throw new IllegalStateException("duplicate internal key across sources");
        current = first.entry();
        return true;
    }

    private void advance(InternalIterator source) {
        if (source.next()) heap.add(new Head(source, source.current()));
    }
    ...
    public void close() {
        if (!closed) {
            closed = true;
            for (InternalIterator source : sources) source.close();
            heap.clear();
        }
    }
