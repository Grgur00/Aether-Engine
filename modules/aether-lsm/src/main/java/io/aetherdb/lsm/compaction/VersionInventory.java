...
public final class VersionInventory {
     ...
     */
    public VersionInventory(List<? extends List<CompactionFile>> levels) {
        this(levels, emptyPointers());
    }
    ...
    public VersionInventory(List<? extends List<CompactionFile>> levels, List<byte[]> pointers) {
        if (levels.size() != LevelCompactionConfig.LEVEL_COUNT || pointers.size() != 6)
            throw new IllegalArgumentException(
                    "exactly seven levels and six pointers are required");
     ...
     */
    public List<CompactionFile> level(int level) {
     ...
     */
    public byte[] pointer(int level) {
        byte[] pointer = pointers.get(level);
        return pointer == null ? null : pointer.clone();
    }
     ...
     */
    public boolean containsFile(long number) {
        return levels.stream().flatMap(List::stream).anyMatch(file -> file.fileNumber() == number);
    }

    private static void validateNonOverlapping(List<CompactionFile> files) {
        for (int i = 1; i < files.size(); i++)
            if (Arrays.compareUnsigned(
                            files.get(i - 1).largestUserKey(), files.get(i).smallestUserKey())
                    >= 0)
                throw new IllegalArgumentException(
                        "nonzero level files must be sorted and non-overlapping");
    }

    private static List<byte[]> emptyPointers() {
        List<byte[]> result = new ArrayList<>(6);
        for (int i = 0; i < 6; i++) result.add(null);
        return result;
    }
}
