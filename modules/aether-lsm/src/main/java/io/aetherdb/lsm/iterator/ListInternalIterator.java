...
public final class ListInternalIterator implements InternalIterator {
    ...
    @Override public boolean next() { ensureOpen(); if (++index < entries.size()) return true; index = entries.size(); return false; }
    @Override public InternalEntry current() { ensureOpen(); if (index < 0 || index >= entries.size()) throw new IllegalStateException("iterator is not positioned"); return entries.get(index); }
