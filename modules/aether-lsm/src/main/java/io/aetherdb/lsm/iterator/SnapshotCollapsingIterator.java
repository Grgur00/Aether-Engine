...
public final class SnapshotCollapsingIterator implements AutoCloseable {
     ...
     */
    public SnapshotCollapsingIterator(
        ...
        if (Arrays.compareUnsigned(startInclusive, endExclusive) > 0)
            throw new IllegalArgumentException("reversed range");
        this.source = source;
        this.visibleSequence = visibleSequence;
        this.startInclusive = startInclusive.clone();
        this.endExclusive = endExclusive.clone();
    }
    ...
    private void ensurePositioned() {
        ensureOpen();
        if (key == null) throw new IllegalStateException("iterator is not positioned");
    }
