...
public final class SnapshotRegistry {
    ...
    private synchronized void release(long sequence) {
        ...
        if (references == 1) registrations.remove(sequence);
        else registrations.put(sequence, references - 1);
        count--;
    }
    ...
    public synchronized long oldestSequence() {
        return registrations.isEmpty() ? -1 : registrations.firstKey();
    }
    ...
    public synchronized long newestSequence() {
        return registrations.isEmpty() ? -1 : registrations.lastKey();
    }
    ...
    public static final class Registration implements AutoCloseable {
        ...
        Registration(SnapshotRegistry registry, long sequence) {
        ...
        public synchronized void close() {
            if (registry != null) {
                SnapshotRegistry owner = registry;
                registry = null;
