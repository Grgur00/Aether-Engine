...
final class CompactionPolicyTest {
    @Test void defaultTargetsAndScoresMatchSpecification() {
        ...
        VersionInventory version = version(List.of(
                file(1, 0, "a", "b", 10), file(2, 0, "c", "d", 10),
                file(3, 0, "e", "f", 10), file(4, 0, "g", "h", 10)), List.of());
        assertEquals(1.0, new CompactionScoreCalculator(config).calculate(version).score(0));
    }

    @Test void l0PickerComputesStableClosureThroughL1() {
        List<CompactionFile> l0 = List.of(file(10, 0, "a", "c", 10), file(11, 0, "f", "h", 10), file(12, 0, "e", "f", 10));
        ...
        CompactionPlan plan = new CompactionPickerV1(LevelCompactionConfig.defaults()).pick(version, scores, 7).orElseThrow();
        assertEquals(List.of(10L, 11L, 12L), plan.primaryInputs().stream().map(CompactionFile::fileNumber).sorted().toList());
        assertEquals(List.of(20L), plan.outputLevelInputs().stream().map(CompactionFile::fileNumber).toList());
        assertArrayEquals(bytes("a"), plan.smallestUserKey());
        assertArrayEquals(bytes("h"), plan.largestUserKey());
    }

    @Test void urgentL0OverridesHigherLevelScore() {
        ...
        CompactionPlan plan = new CompactionPickerV1(LevelCompactionConfig.defaults())
                .pick(version, new CompactionScores(new double[] {3, 9, 0, 0, 0, 0}), 0).orElseThrow();
        assertEquals(0, plan.inputLevel());
        assertEquals(CompactionReason.URGENT_L0, plan.reason());
    }

    @Test void reclamationKeepsNewerVersionsAndOneBoundaryVersion() {
        var source = new ListInternalIterator(List.of(
                value("a", 9), value("a", 7), value("a", 5), value("a", 2),
                InternalEntry.tombstone(bytes("b"), 4), value("b", 1)));
        List<Long> retained = new ArrayList<>();
        try (var dropping = new CompactionDroppingIterator(source, 5, key -> new String(key, UTF_8).equals("b"))) {
    ...
    @Test void baseLevelCheckSearchesOnlyLevelsBelowOutput() {
    ...
    @Test void rangeRegistrationRejectsConflictsAndClosesIdempotently() {
        CompactionRangeRegistry registry = new CompactionRangeRegistry();
        var registration = registry.register(1, 2, bytes("a"), bytes("m"));
        assertThrows(IllegalStateException.class, () -> registry.register(2, 3, bytes("m"), bytes("z")));
    ...
    private static VersionInventory version(List<CompactionFile> l0, List<CompactionFile> l1) {
        List<List<CompactionFile>> levels = emptyLevels(); levels.set(0, l0); levels.set(1, l1); return new VersionInventory(levels);
    }
    private static List<List<CompactionFile>> emptyLevels() {
        List<List<CompactionFile>> result = new ArrayList<>(); for (int i = 0; i < 7; i++) result.add(List.of()); return result;
    }
    private static CompactionFile file(long number, int level, String smallest, String largest, long size) {
        return new CompactionFile(number, level, bytes(smallest), bytes(largest), size);
    }
    private static InternalEntry value(String key, long sequence) { return InternalEntry.value(bytes(key), sequence, bytes("v")); }
